        ...
        <div className="flex flex-col h-full">
            ...
            <div className="flex items-center gap-2 p-3 border-b border-black/[0.06]">
                ...
                <button
                    ...
                    className="px-2.5 py-1 text-xs font-medium bg-emerald-50 text-emerald-600 rounded-lg hover:bg-emerald-100 transition-colors disabled:opacity-30"
                >
                    Add
                </button>
            </div>
            ...
            <div className="flex items-center gap-1 px-3 py-2 border-b border-black/[0.04]">
                ...
                <span className="ml-auto text-xs text-zinc-400">{filtered.length} tasks</span>
            </div>
                        ...
                        {filtered.map((task) => (
                            ...
                            >
                                <div className="flex items-center gap-3 px-3 py-2.5 group">
                                    ...
                                    >
                                        {task.status === 'done' ? <CheckSquare size={16} className="text-emerald-500" /> : <Square size={16} />}
                                    </button>
                                    <div className="flex-1 min-w-0">
                                        <p className={cn('text-sm truncate', task.status === 'done' ? 'text-zinc-400 line-through' : 'text-zinc-700')}>
                                            {task.title}
                                        </p>
                                    ...
                                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <button onClick={() => setExpandedId(expandedId === task.id ? null : task.id)} className="p-1 text-zinc-400 hover:text-zinc-600">
                                            {expandedId === task.id ? <ChevronUp size={14} /> : <ChevronDown size={14} />}
                                        </button>
                                        <button onClick={() => deleteTask(task.id)} className="p-1 text-zinc-400 hover:text-rose-500">
                                            <Trash2 size={14} />
                                        </button>
                                    </div>
                                </div>
                                                ...
                                                {editingId === task.id ? (
                                                    <div className="flex gap-2">
                                                        <input
                                                            ...
                                                            className="flex-1 bg-zinc-100 rounded-lg px-2.5 py-1.5 text-xs text-zinc-700 outline-none border border-black/[0.06]"
                                                            placeholder="Description..."
                                                        />
                                                        <button
                                                            onClick={() => { updateTask(task.id, { description: editDesc }); setEditingId(null); }}
                                                            className="px-2 py-1 text-xs bg-emerald-50 text-emerald-600 rounded-lg"
                                                        >
                                                            Save
                                                        </button>
                                                    </div>
                                                    ...
                                                    {(['low', 'medium', 'high'] as const).map((p) => (
                                                        <button
                                                            ...
                                                            className={cn(
                                                                ...
                                                                task.priority === p ? PRIORITY_COLORS[p] : 'text-zinc-400 hover:text-zinc-600'
                                                            )}
                                                        >
                                                            {p}
                                                        </button>
                                                ...
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    {(['todo', 'in_progress', 'done'] as const).map((s) => (
                                                        <button
                                                            ...
                                                            className={cn(
                                                                'text-[10px] px-1.5 py-0.5 rounded capitalize',
                                                                task.status === s ? 'bg-black/[0.06] text-zinc-700' : 'text-zinc-400 hover:text-zinc-600'
                                                            )}
                                                        >
        ...
        <div className="flex flex-col items-center justify-center h-full gap-6 p-6">
            ...
            <div className="relative w-52 h-52">
                <svg className="w-full h-full -rotate-90" viewBox="0 0 200 200">
                    <circle cx="100" cy="100" r="90" fill="none" stroke="black" strokeOpacity="0.04" strokeWidth="6" />
                    <motion.circle
                        cx="100" cy="100" r="90"
                        fill="none"
                        stroke={strokeColor}
            ...
            {currentTask && (
                <p className="text-xs text-zinc-500 truncate max-w-[200px]">
                    Working on: <span className="text-zinc-700">{currentTask.title}</span>
                </p>
            )}

            {/* Controls */}
            <div className="flex items-center gap-3">
                <button onClick={resetPomodoro} className="p-2.5 rounded-xl bg-black/[0.04] text-zinc-400 hover:text-zinc-600 hover:bg-black/[0.08] transition-colors">
                    <RotateCcw size={18} />
                </button>
                {!pomodoro.isRunning ? (
                    <button
                ...
                <button onClick={skipToNext} className="p-2.5 rounded-xl bg-black/[0.04] text-zinc-400 hover:text-zinc-600 hover:bg-black/[0.08] transition-colors">
                    <SkipForward size={18} />
                </button>
            </div>
            ...
            <div className="flex items-center gap-4 text-xs text-zinc-500">
                <label className="flex items-center gap-1.5">
                    ...
                    <input
                        ...
                        className="w-10 bg-zinc-100 rounded px-1.5 py-0.5 text-zinc-700 text-center outline-none border border-black/[0.06]"
                    />
                    min
                </label>
                <label className="flex items-center gap-1.5">
...
const AutomationTab: React.FC = () => {
    ...
    const handleSubmit = () => {
        ...
        } else if (form.actionType === 'authorize_app') {
            if (form.params.app_id) params.app_id = form.params.app_id;
        }
        addRule({
            name: form.name,
    ...
    const renderParamFields = () => {
        switch (form.actionType) {
            case 'send_notification':
                return (
                    <>
                        <input value={form.params.title} onChange={(e) => setForm({ ...form, params: { ...form.params, title: e.target.value } })} placeholder="Notification title" className="w-full bg-zinc-100 rounded-lg px-2.5 py-1.5 text-xs text-zinc-700 outline-none border border-black/[0.06]" />
                        <input value={form.params.message} onChange={(e) => setForm({ ...form, params: { ...form.params, message: e.target.value } })} placeholder="Notification message" className="w-full bg-zinc-100 rounded-lg px-2.5 py-1.5 text-xs text-zinc-700 outline-none border border-black/[0.06]" />
                    </>
                );
            case 'open_app':
                return (
                    <select value={form.params.app_id} onChange={(e) => setForm({ ...form, params: { ...form.params, app_id: e.target.value } })} className="w-full bg-zinc-100 rounded-lg px-2 py-1.5 text-xs text-zinc-700 outline-none border border-black/[0.06]">
            ...
            case 'create_task':
                return (
                    <>
                        <input value={form.params.title} onChange={(e) => setForm({ ...form, params: { ...form.params, title: e.target.value } })} placeholder="Task title" className="w-full bg-zinc-100 rounded-lg px-2.5 py-1.5 text-xs text-zinc-700 outline-none border border-black/[0.06]" />
                        <select value={form.params.priority} onChange={(e) => setForm({ ...form, params: { ...form.params, priority: e.target.value } })} className="w-full bg-zinc-100 rounded-lg px-2 py-1.5 text-xs text-zinc-700 outline-none border border-black/[0.06]">
            ...
            case 'run_command':
                return (
                    <input value={form.params.command} onChange={(e) => setForm({ ...form, params: { ...form.params, command: e.target.value } })} placeholder="Shell command (e.g. npm run build)" className="w-full bg-zinc-100 rounded-lg px-2.5 py-1.5 text-xs text-zinc-700 outline-none border border-black/[0.06] font-mono" />
                );
            case 'navigate_url':
                return (
                    <input value={form.params.url} onChange={(e) => setForm({ ...form, params: { ...form.params, url: e.target.value } })} placeholder="URL (e.g. https://example.com)" className="w-full bg-zinc-100 rounded-lg px-2.5 py-1.5 text-xs text-zinc-700 outline-none border border-black/[0.06]" />
                );
            case 'send_to_board':
                return (
                    <>
                        <input value={form.params.title} onChange={(e) => setForm({ ...form, params: { ...form.params, title: e.target.value } })} placeholder="Card title" className="w-full bg-zinc-100 rounded-lg px-2.5 py-1.5 text-xs text-zinc-700 outline-none border border-black/[0.06]" />
                        <input value={form.params.content} onChange={(e) => setForm({ ...form, params: { ...form.params, content: e.target.value } })} placeholder="Card content" className="w-full bg-zinc-100 rounded-lg px-2.5 py-1.5 text-xs text-zinc-700 outline-none border border-black/[0.06]" />
                        <select value={form.params.color} onChange={(e) => setForm({ ...form, params: { ...form.params, color: e.target.value } })} className="w-full bg-zinc-100 rounded-lg px-2 py-1.5 text-xs text-zinc-700 outline-none border border-black/[0.06]">
        ...
        <div className="flex flex-col h-full">
            <div className="flex items-center justify-between p-3 border-b border-black/[0.06]">
                ...
                <button
                    ...
                    className="flex items-center gap-1.5 px-2.5 py-1 text-xs font-medium bg-violet-50 text-violet-600 rounded-lg hover:bg-violet-100 transition-colors"
                >
            ...
            <AnimatePresence>
                {showForm && (
                    <motion.div
                        ...
                        animate={{ height: 'auto', opacity: 1 }}
                        exit={{ height: 0, opacity: 0 }}
                        className="overflow-hidden border-b border-black/[0.06]"
                    >
                        <div className="p-3 space-y-2.5">
                            <input
                                ...
                                className="w-full bg-zinc-100 rounded-lg px-2.5 py-1.5 text-xs text-zinc-700 outline-none border border-black/[0.06]"
                            />
                            <div className="flex gap-2">
                                <select
                                    value={form.triggerType}
                                    onChange={(e) => setForm({ ...form, triggerType: e.target.value as any })}
                                    className="flex-1 bg-zinc-100 rounded-lg px-2 py-1.5 text-xs text-zinc-700 outline-none border border-black/[0.06]"
                                >
                                ...
                                {form.triggerType === 'event' ? (
                                    ...
                                    <div className="flex items-center gap-1.5">
                                        ...
                                        <input
                                            ...
                                            className="w-14 bg-zinc-100 rounded px-2 py-1.5 text-xs text-zinc-700 text-center outline-none border border-black/[0.06]"
                                        />
                            ...
                            <select
                                ...
                                className="w-full bg-zinc-100 rounded-lg px-2 py-1.5 text-xs text-zinc-700 outline-none border border-black/[0.06]"
                            >
                                ...
                                <option value="authorize_app">Authorize Integration</option>
                            </select>
                            {renderParamFields()}
                            <div className="flex gap-2">
                                <button onClick={() => setShowForm(false)} className="flex-1 px-2.5 py-1.5 text-xs text-zinc-400 hover:text-zinc-600 rounded-lg bg-zinc-100">
                                    Cancel
                                </button>
                                <button onClick={handleSubmit} className="flex-1 px-2.5 py-1.5 text-xs font-medium bg-violet-50 text-violet-600 rounded-lg hover:bg-violet-100">
                                    Create
                                </button>
                            </div>
                        </div>
                ...
                {automationRules.length === 0 && !showForm ? (
                    ...
                    <AnimatePresence>
                        {automationRules.map((rule) => (
                            ...
                            >
                                <button onClick={() => toggleRule(rule.id)} className={cn('transition-colors', rule.enabled ? 'text-emerald-500' : 'text-zinc-400')}>
                                    <Power size={14} />
                                </button>
                                <div className="flex-1 min-w-0">
                                    <p className={cn('text-sm truncate', rule.enabled ? 'text-zinc-700' : 'text-zinc-400')}>
                                        {rule.name}
                                    </p>
...
export const TasksApp: React.FC<TasksAppProps> = ({ windowData }) => {
    ...
    return (
        <div className="flex flex-col h-full bg-white text-zinc-900">
            {/* Tab bar */}
            <div className="flex items-center gap-0.5 px-2 pt-2 pb-0 border-b border-black/[0.06]">
                {TABS.map((tab) => {
                    const Icon = tab.icon;
